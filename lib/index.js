  ...
  var onParse = function onParse() {
    if (value) {
      try {
        ...
        secondVal = _cronRegex.secondRegex.test(secondVal) ? secondVal : '0';
        minuteValue = _cronRegex.minuteRegex.test(minuteValue) ? minuteValue : '0';
        hourVal = _cronRegex.hourRegex.test(hourVal) ? hourVal : '0';
        dayVal = _cronRegex.dayRegex.test(dayVal) ? dayVal : '1';
